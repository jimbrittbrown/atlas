export interface NarrationCategoryScores {
  naturalFlow: number;
  emotionalDelivery: number;
  pacing: number;
  clarity: number;
  listenerEngagement: number;
  overallNarrationQuality: number;
}

export interface NarrationEvaluationResult {
  scores: NarrationCategoryScores;
  overallScore: number;
  diagnosis: string;
...
export class NarrationEvaluator {
  evaluate(plan: NarrationPlanInput): NarrationEvaluationResult {
    const normalized = this.normalize(plan);

    const categoryScores: Omit<NarrationCategoryScores, 'overallNarrationQuality'> = {
    ...
    const overallNarrationQuality = Math.max(0, Math.min(10, Math.round(overallCore)));

    const scores: NarrationCategoryScores = {
      ...categoryScores,
      overallNarrationQuality
    };

    return {
      scores,
      overallScore: overallCore,
      diagnosis: this.buildDiagnosis(scores, overallCore),
      recommendedImprovements: this.buildImprovements(scores),
      classification: this.classify(scores, overallCore)
    };
  }

  private normalize(plan: NarrationPlanInput): Required<NarrationPlanInput> {
    return {
  ...
  private scoreNaturalFlow(plan: Required<NarrationPlanInput>): number {
    let score = 3;
  ...
  private scoreEmotionalDelivery(plan: Required<NarrationPlanInput>): number {
    let score = 3;
  ...
  private scorePacing(plan: Required<NarrationPlanInput>): number {
    let score = 3;
  ...
  private scoreClarity(plan: Required<NarrationPlanInput>): number {
    let score = 3;
  ...
  private scoreListenerEngagement(plan: Required<NarrationPlanInput>): number {
    let score = 3;
  ...
  private buildDiagnosis(scores: NarrationCategoryScores, overall: number): string {
  ...
  private buildImprovements(scores: NarrationCategoryScores): string[] {
  ...
  private classify(scores: NarrationCategoryScores, overall: number): 'PASS' | 'CONDITIONAL' | 'FAIL' {
    const critical = [
      scores.naturalFlow,
      scores.pacing,
      scores.clarity,
