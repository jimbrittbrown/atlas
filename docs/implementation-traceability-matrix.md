...
| ECP-003 | COMPLETE | Credential operations evidence executed with deterministic drill artifacts and validator; live-like rotation and revocation passed, but direct custody recovery remained only partial across repository/runtime-adjacent classes and failed for the unresolved vault-backed classes, so ECR-R3 stays open. |
