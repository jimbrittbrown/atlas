...
async function createValidationSystem() {
  ...
  const customer = missionControl.customerRegistry.createCustomer({
    companyName: 'Website Production Validation',
    contactName: 'Atlas Ops',
    email: 'ops@website-production-validation.example',
    phone: '+1-555-0123',
    website: 'https://website-production-validation.example',
    industry: 'Media',
    adapterType: 'FRAMER',
  ...
  const proposal = planning.submitProposal({
    sourceType: 'CEO',
    sourceId: 'website-production-validation',
    customerId: customer.customerId,
    title: 'Website Production Manager Validation Proposal',
    description: 'Validate production QA and delivery package generation.',
  ...
  planning.applyDecision({
    ...
    decidedBy: 'CEO',
    rationale: 'Approved for website production manager validation.',
    conditions: []
  });
...
async function main() {
  ...
  const report = {
    ...
    deliveryPackageSummary: {
      qaScore: evaluation.review?.deliveryPackage?.qaResults?.score ?? 0,
      confidenceScore: evaluation.review?.deliveryPackage?.confidenceScore ?? 0,
      recommendedRevisions: evaluation.review?.deliveryPackage?.recommendedRevisions ?? []
    },
    dashboardSummary: {
    ...
    remainingLimitations: [
      'Screenshot capture is generated as governed tasks and references; image files are not captured automatically in v1.',
      'Responsive verification relies on metadata signals from Framer read surfaces.'
    ]
  };

  mkdirSync('/root/atlas/review', { recursive: true });
  writeFileSync('/root/atlas/review/website-production-manager-v1-report.json', JSON.stringify(report, null, 2));
  writeFileSync('/root/atlas/review/website-production-manager-v1-report.md', toMarkdown(report));

  console.log('Website Production Manager v1 validation completed.');
  console.log('/root/atlas/review/website-production-manager-v1-report.json');
  console.log('/root/atlas/review/website-production-manager-v1-report.md');
}
